...
- `roh:Reservable`, where an organization may `roh:hasReservable`, belonging to any of its subclasses, e.g. `roh:Equipment`, `roh:Facility` or `obo-ero:ERO\_0000071` (Software).
- `foaf:Organization`, where an organization may be linked through `vivo:hasSucessorOrganization` or `vivo:hasPredecessorOrganization` with another `foaf:Organization` or may be part of (`obo-ro:BFO\_0000050`) or include `obo-ro:BFO\_0000051` (has part) other several `foaf:Organization`s.
